...
let PersonaBajaComponent = class PersonaBajaComponent {
    ...
    _getCapacitaciones() {
        ...
        this.eventoService.getAll().toPromise()
            ...
            this.message.error('Ocurrió un error al recuperar lista de capacitaciones');
            this.procesando = false;
        });
    }
    _createForm() {
...
PersonaBajaComponent.ctorParameters = () => [
...
let PersonaCargaComponent = class PersonaCargaComponent {
    constructor(msg, http, eventoService) {
        this.msg = msg;
        this.http = http;
        this.eventoService = eventoService;
        this.procesando = false;
        ...
        this.filters = [
            {
                ...
                fn: (fileList) => {
                    ...
                    if (filterFiles.length !== fileList.length) {
                        this.msg.error(`Solo se admiten archivos del tipo csv`);
                        return filterFiles;
                    }
                    return fileList;
                }
        ...
        this.handleUpload = (item) => {
            ...
            formData.append('file', item.file, this.uploadFileName);
            // const url_app = 'http://104.41.14.101:8083/api/personal/upload/';
            const url_app = src_environments_environment_prod__WEBPACK_IMPORTED_MODULE_5__["environment"].host + 'personal/upload/';
            formData.append('usuarioEjecucion', 'VPEREZ');
            // this.http.post('https://jsonplaceholder.typicode.com/posts/', formData).subscribe(
            this.http.post(url_app, formData).subscribe(res => {
                console.log("success", JSON.stringify(res));
                this.listaPersonal = res['data'] || [];
                this.idProcesoSeleccionado = Number(res['message']);
                item.onSuccess(item.file);
                this.procesando = false;
                this.verListaPersonal = true;
            }, (err) => {
    ...
    matricular() {
        ...
        this.eventoService.matricular(this.eventoSeleccionado, this.idProcesoSeleccionado).toPromise()
            .then(res => {
            if (res['codigo'] == 100) {
                this.msg.success("La operación fue realizada con éxito");
            }
            else {
                this.msg.error('ocurrió un error al realizar esta operación');
            }
            this.procesando = false;
        }, err => {
            this.procesando = false;
    ...
    seleccionarEvento(idEvento) {
        ...
        this.listaEventos.forEach(e => {
            if (e['idEvento'] == this.eventoSeleccionado) {
                this.textoEventoSeleccionado = e['nombreEvento'];
            }
        });
    }
};
PersonaCargaComponent.ctorParameters = () => [
...
let PersonaEdicionComponent = class PersonaEdicionComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let PersonaListarComponent = class PersonaListarComponent {
    constructor(
...
PersonaListarComponent.ctorParameters = () => [
    { type: ng_zorro_antd__WEBPACK_IMPORTED_MODULE_2__["NzMessageService"] },
...
let PersonaNuevoComponent = class PersonaNuevoComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let PersonasRoutingModule = class PersonasRoutingModule {
};
...
let PersonasShellComponent = class PersonasShellComponent {
    constructor() {
...
let UploadPersonasComponent = class UploadPersonasComponent {
    constructor() {
        this.isCollapsed = false;
        this.options = [
            {
